...
import { addGuestOrder, addGuestRentalOrder } from "../../redux/slices/guestOrderSlice";
...
const CheckoutBtn = ({
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (response) {
        if (!token) {
          type === 'rent' ? dispatch(addGuestOrder(response.data)) : dispatch(addGuestRentalOrder(response.data))
